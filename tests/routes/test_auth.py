...
def test_refresh_token_user_not_found(client, monkeypatch):
    ...
    login_resp = client.post(
        "/auth/token",
        data={"username": "nouser@example.com", "password": "testpass"}
    )
    refresh_token = login_resp.json()["refresh_token"]
    monkeypatch.setattr(
        "app.repositories.refresh_token_repository.RefreshTokenRepository.get_by_token",
...
def test_refresh_token_revoked(client, monkeypatch):
    ...
    client.post(
        "/auth/register",
        json={"name": "Revoked", "email": "revoked@example.com", "password": "testpass"}
    )
    login_resp = client.post(
        "/auth/token",
        data={"username": "revoked@example.com", "password": "testpass"}
    )
    refresh_token = login_resp.json()["refresh_token"]
    monkeypatch.setattr(
        "app.repositories.refresh_token_repository.RefreshTokenRepository.get_by_token",
...
def test_refresh_token_success(client, monkeypatch):
    ...
    client.post(
        "/auth/register",
        json={"name": "User Success", "email": "success@example.com", "password": "testpass"}
    )
    login_resp = client.post(
        "/auth/token",
        data={"username": "success@example.com", "password": "testpass"}
    )
    refresh_token = login_resp.json()["refresh_token"]
